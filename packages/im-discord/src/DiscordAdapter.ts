/**
 * DiscordAdapter —— Discord Gateway WebSocket 适配器(ImAdapter seam 的实现)
 *
 * 纯网络协议层(harness-agnostic):
 * - transport 抽象与飞书同构:connect(handlers) → { disconnect, selfId };
 *   Gateway 协议(HELLO/IDENTIFY/HEARTBEAT/RECONNECT/序列号/resume)由生产 transport 实现;
 * - 入站:gateway 帧 → 规范化 → 覆盖通道名 → 幂等去重 → onInbound(异常隔离);
 * - 出站:sendText 分段(2000 硬限制);sendCard 渲染 embed + components。
 */
...
import { discordFrameToMessage } from './events.ts'
...
/** 传输层抽象(生产实现:Gateway WS 客户端 + HTTP send message) */
export interface DiscordTransport {
  ...
  sendMessage(channelId: string, payload: { content?: string; embeds?: unknown[]; components?: unknown[] }): Promise<void>
}
...
export class DiscordAdapter implements ImAdapter {
  ...
  constructor(opts: DiscordAdapterOptions) {
    this.channel = opts.channel ?? 'discord'
    this.opts = opts
  }
  ...
  async sendText(target: PeerRef, text: string): Promise<void> {
    ...
    for (const segment of segments) {
      await this.opts.transport.sendMessage(target.id, { content: segment.text })
    }
  }

  /** 卡片出站:embed + components */
  async sendCard(target: PeerRef, card: AnyCard): Promise<void> {
    const payload = renderCard(card)
    await this.opts.transport.sendMessage(target.id, { embeds: payload.embeds, components: payload.components })
  }
  ...
  /** 入站:帧 → 规范化 → 覆盖通道名 → 去重 → 回调(异常隔离) */
  private handleEvent(payload: unknown): void {
    const result = discordFrameToMessage(payload, this.selfIdValue)
    if (!result.ok) return
