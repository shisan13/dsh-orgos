/**
 * Slack 长消息分段(单条消息上限约 40000,取 30000 保守;纯函数)
 *
 * 复用 §9.1 通用分段规则:行边界优先、代码块不拆、表格转列表(mrkdwn 不支持表格)。
 */
